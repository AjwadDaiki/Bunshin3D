        ...
        <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider flex items-center gap-2">
          <div
            className={cn(
              ...
              isGenerating ? "bg-green-400 animate-pulse" : "bg-neutral-600",
            )}
          />
          {t("Logs.title")}
        </h3>
        <span className="text-[11px] text-neutral-500 bg-white/5 border border-white/6 rounded-full px-2 py-0.5">
          {logs.length}
        </span>
      </div>
      <div
        ref={logsContainerRef}
        className="h-48 overflow-y-auto space-y-2 font-mono text-sm pr-2 scrollbar-thin scrollbar-thumb-white/10 scroll-smooth"
      >
        {logs.length === 0 ? (
